 ...
 */
@TargetApi(18)
public class CustomDrmSessionManager<T extends ExoMediaCrypto> implements DrmSessionManager<T>,
        CustomDrmSession.ProvisioningManager<T> {
   ...
   * @throws UnsupportedDrmException If the specified DRM scheme is not supported.
   */
  public static CustomDrmSessionManager<FrameworkMediaCrypto> newPlayReadyInstance(
   ...
   *     to {@link ExoMediaDrm#getKeyRequest(byte[], List, int, HashMap)}. May be null.
   */
  public CustomDrmSessionManager(
      UUID uuid,
  ...
  public CustomDrmSessionManager(
    ...
    if (multiSession && C.WIDEVINE_UUID.equals(uuid) && Util.SDK_INT >= 19) {
      ...
      // useful if CustomDrmSession instances were aware of one another's state, which is not
  ...
  public DrmSession<T> acquireSession(Looper playbackLooper, DrmInitData drmInitData) {
    ...
    CustomDrmSession<T> session;
    if (!multiSession) {
      session = sessions.isEmpty() ? null : sessions.get(0);
    } else {
      // Only use an existing session if it has matching init data.
      session = null;
      for (CustomDrmSession<T> existingSession : sessions) {
  ...
  @Override
  public void provisionRequired(CustomDrmSession<T> session) {
  ...
  public void onProvisionCompleted() {
    for (CustomDrmSession<T> session : provisioningSessions) {
  ...
  private class MediaDrmHandler extends Handler {
    ...
    public void handleMessage(Message msg) {
      ...
      for (CustomDrmSession<T> session : sessions) {
